...
1. `make`
2. `python [example name]` (e.g. `python paper_example.py`)
...
The simplest way to use DAPT is with a delimiter-separated file, such as a CSV.  CSVs are common files used to store two-dimensional data, typically with a header.  This makes them natural to use as a "database", where rows are tests and columns are the attributes. This example is by executing 'python paper_example.py'.
...
This table is saved in [parameters.csv](/parameters.csv) and used by [paper_example.py](/paper_example.py).  This file isn't modified by the script.  The contents are copied to a file named `basic_params.csv` so the master database doesn't get overwritten.  Run the script (`python basic.py`) and observe the results.  The outputs folder now contains three final output SVGs from the script.
